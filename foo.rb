...
#  - If a word is present in document, it is marked as '1', otherwise
# '0'
...
# We're not sure which Kernel will perform best, so let's give each a
# try
...
kernels.each_index { |j|
  ...
  labels.each_index { |i|
    ...
    #puts "Prediction: #{pred}, True label: #{labels[i]}, Kernel:
    ##{kernel_names[j]}"
  ...
  test_labels.each_index { |i|
    ...
    puts "\t Prediction: #{pred}, Probs: #{probs}, True label: #{test_labels[i]}"
